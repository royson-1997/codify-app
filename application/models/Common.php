...
class Common extends CI_Model
{
  ...
  function getdata($table_name,$where_clause=array()){
 ...
 function getdataInArray($table_name,$column='',$where_clause=array()){
  ...
  function delete($table_name,$data,$where_clause=array()){
    if(count($where_clause) > 0){
      $this->db->where($where_clause);
    }
  ...
  function add_data($tablename,$data=array(),$action){

    $dat = array_filter($data);
    $this->db->$action($tablename,$dat);
    return true;
